...
export function Editor({ range, draft, onClose, onCreated, onPatched }: EditorProps) {
  ...
  const formatTimeRange = () => {
    ...
    // Format in MSK timezone
    const mskOffset = 3 * 60 * 60 * 1000;
    const startMsk = new Date(start.getTime() + mskOffset);
    const endMsk = new Date(end.getTime() + mskOffset);
    
    const formatTime = (date: Date) => 
      date.toISOString().slice(11, 16); // HH:MM format
    
    const formatDate = (date: Date) => 
      date.toISOString().slice(0, 10); // YYYY-MM-DD format
        ...
        <div className="space-y-4">
          <div>
            <input
              ...
              className="w-full px-3 py-2 bg-zinc-800 border border-zinc-600 rounded text-white placeholder-zinc-400 focus:outline-none focus:border-zinc-400"
              autoFocus
            />
          </div>
            ...
            <>
              <div>
                <textarea
                  ...
                  rows={3}
                  className="w-full px-3 py-2 bg-zinc-800 border border-zinc-600 rounded text-white placeholder-zinc-400 focus:outline-none focus:border-zinc-400 resize-none"
                />
              </div>
              ...
              {showReflection && (
                <div className="space-y-3 p-3 bg-zinc-800 rounded border border-zinc-600">
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full px-2 py-1 bg-zinc-700 border border-zinc-600 rounded text-white placeholder-zinc-400 focus:outline-none focus:border-zinc-400 resize-none text-sm"
                  />
                </div>
              )}
            </div>
